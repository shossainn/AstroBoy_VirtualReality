...
public class BreakWindow : MonoBehaviour {
    ...
    private void OnTriggerEnter(Collider other) {
        if(other.gameObject.tag == "hammer" && !broken){
            ...
            OVRGrabbable otherGrabbable = other.gameObject.GetComponentInChildren<OVRGrabbable>();
            if (otherGrabbable != null && otherGrabbable.grabbedBy != null)
            {
                otherGrabbable.grabbedBy.hapticPulse();
            }
